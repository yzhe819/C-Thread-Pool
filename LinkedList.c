...
struct LinkedList_int linked_list_int_constructor() {
    struct LinkedList_int new_linked_list;
    new_linked_list.head = NULL;
    new_linked_list.length = 0;

    new_linked_list.insert = insert_node_int;
    new_linked_list.remove = remove_node_int;
    new_linked_list.retrieve = retrieve_data_int;

    return new_linked_list;
}

struct Node_int* create_node_int(int data) {
    struct Node_int* new_node_address;
    // Allocate memory for the new node
    new_node_address = (struct Node_int*)malloc(sizeof(struct Node_int));
